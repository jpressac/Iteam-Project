...
public class IteamUserDetailService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		GetResponse response = elasticsearchRepository.checkUser("user", "data", username);
		if (response.isExists()) {
			String userRoles = response.getField("roles").getValue().toString();
			return new User(response.getId(), response.getField("password").getValue().toString(),
					AuthorityUtils.createAuthorityList(userRoles));
		} else {
			return new User("admin", "admin", AuthorityUtils.createAuthorityList(new String[] { "ROLE_ADMIN" }));
		}
	}
